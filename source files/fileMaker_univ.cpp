...
void tofile(int* x,int size,int sample,double percent){
    string filename = string("Your_path")+"wyniki"+"/"+"not_sorted"+"/"+to_string(size)+"/"+to_string((int)(percent*1000))+"/"+ "array_" +to_string(size)+"_"+to_string(sample+1)+"_"+to_string((int)(percent*1000))+ ".txt";
...
void resultstofile(long long duration, int size,string type,double percent) {
    string filename = string("Your_path")+"wyniki/" + type + "/results"+"/"+ "results_"+type+"_"+ to_string(size)+"_"   + to_string((int)(percent*1000)) + ".txt";
...
void sortedtofile(int* x,int size,string type,int sample,double percent){
    string filename = string("Your_path") +"wyniki/"+type+"/"+"sorted"+"/"+to_string(size)+"/"+to_string((int)(percent*1000))+"/"+ "sorted_array_" +to_string(size)+"_"+ to_string(sample)+"_"+ to_string((int)(percent*1000))+ ".txt";
    ofstream myfile (filename);
    for(int i = 0; i < size; i ++){
        myfile << x[i] << " " ;
    }
...
void tocsv(string type, int* sizes, int sizes_len, double* percentages, int percentages_len) {
    ofstream outfile(string("Your_path") + "wynik" + type + ".csv");
    ...
    for (int i = 0; i < sizes_len; i++) {
        ...
        for (int j = 0; j < percentages_len; j++) {
            ...
            string filename = string("Your_path") + "wyniki/" + type + "/results" + "/" + "results_" + type + "_" + to_string(size) + "_" + to_string((int)(p * 1000)) + ".txt";
